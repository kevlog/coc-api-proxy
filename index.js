...
import askQuestion from "./create-env.js";

const app = express();

async function main() {
  if (!process.env.PORT || !process.env.COC_API_KEY) {
    await askQuestion();
  }
  ...
  app.get("/api/clan", async (req, res) => {
    try {
      ...
      if (clanTag.startsWith("#")) {
        return res
          .status(400)
          .json({ error: "Please enter the clan tag without '#'." });
      }
      ...
      const url = `https://api.clashofclans.com/v1/clans/${encodeURIComponent(
        clanTag
      )}`;
      ...
      if (!response.ok) {
        ...
        return res
          ...
          .json({ error: JSON.parse(errorText) });
      }
